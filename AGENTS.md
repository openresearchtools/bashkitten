...
- Confirmed deletion of session directories, including all chats grouped under an exact project folder, and title-file renaming from sidebar secondary-click menus; see `docs/chat-management-2026-09-07.md`. Deletion never targets the working directory.
...
- Sending text messages and attachments of any file type to sessions. Attachments are copied into the session's private `attachments/` directory under their sanitized original filenames. Every attached file is identified to the agent by both filename and absolute saved path so it can inspect the file with its tools; supported images are additionally sent inline to image-capable models. The composer accepts files through its picker, paste, or drag-and-drop directly onto the message box. Pending attachments appear inside the composer above its text, and sent attachments appear above their message text. Every persisted attachment has an authenticated download action.
- Showing a folder set/change control instead of a thread title in the chat header. Use the same folder-only picker for new and existing sessions; it lists subfolders, moves to the parent, accepts an absolute path, and can create one new subfolder. Existing-session changes wait until the active turn settles, then the worker changes its tool/system-prompt directory and atomically updates the current JSONL header plus a `bashkitten.cwd` custom event. Retain the segment's initial directory for historical forks. Sidebar regrouping uses the updated header, without a new index or moving session storage.
- Keeping the New chat button text-only and the message composer square-cornered.
...
- Rendering images returned by `read` or any other tool as a compact `Viewed …` activity and rendering inline message images as thumbnails. Clicking any transcript or tool-result image opens a viewer over only the main chat pane, leaving the sidebar visible. The viewer has close, zoom-out, and zoom-in controls; zoomed images larger than the pane scroll horizontally and vertically.
- Providing Copy and Fork actions on each persisted user or assistant message. Copy places that message's text and attachment references on the clipboard. Fork asks the Rust server to atomically create a new session containing the original JSONL history exactly through the selected message, copies only attachments referenced by that retained history, records the source as `parentSession`, and returns the new session ID. JavaScript must never duplicate or rewrite session history itself; it only requests the fork and opens the returned session.
...
Web authentication is local implementation plumbing, not a collection of user-facing security choices. Use the following fixed conservative defaults:

- Hash the password with Argon2id and a unique salt generated by the operating system's cryptographically secure random-number generator. Store the encoded password hash; never store or log the plaintext password.
- Generate each login session token as 32 random bytes from the operating system's cryptographically secure random-number generator. Send the opaque token only in the session cookie and persist only a cryptographic hash of it on the server.
- Login sessions survive a Web UI process restart and expire 30 days after creation. Logout invalidates that individual server-side session. Resetting the Web UI user invalidates every session and removes the stored username and password hash.
- Set the session cookie with `HttpOnly`, `SameSite=Strict`, and `Path=/`, without a `Domain` attribute. Set `Secure` whenever the Web UI is served over HTTPS; do not falsely set it for the default plain HTTP `127.0.0.1` deployment. Never place authentication or provider tokens in local storage, session storage, URLs, HTML, or JavaScript-readable cookies.
- Require an authenticated session for every session-history, attachment, provider, model, control, and live-stream endpoint. Authentication must be enforced by the Rust server rather than trusted to Web UI JavaScript.
- Use authenticated Server-Sent Events for server-to-browser live output and ordinary authenticated HTTP requests for browser-to-server messages and controls. Do not require a bidirectional WebSocket merely for chat streaming.
- Every state-changing endpoint must use `POST`, `PUT`, `PATCH`, or `DELETE`; no `GET` request may change state. Require an unguessable per-login CSRF token in a custom request header and verify that the request's `Origin` exactly matches the configured Web UI origin. Reject cross-origin state-changing requests.
- Generate the CSRF token from the operating system's cryptographically secure random-number generator, associate only its hash with the login session, and expose the original token to authenticated same-origin Web UI code without placing it in a URL or persistent browser storage.
- Protect first-time signup with an exclusive process lock and atomic create-if-absent credentials-file operation. Recheck that no user exists while holding the lock. Exactly one concurrent signup request may succeed.
- Create BashKitten configuration, credential, session, and attachment directories with mode `0700`. Create credential files, session JSONLs, title files, and attachments with mode `0600`. Create control sockets for access by the owning user only. Correct overly broad permissions before using an existing sensitive file, or refuse to use it with a clear error when correction fails.
- Compare password-derived values, token hashes, and CSRF-token hashes using constant-time comparison where applicable. Do not return responses that distinguish an unknown username from a wrong password.
...
- Pi-compatible Codex prompt caching across turns and worker restarts: use the same stable session ID as Pi's clamped `prompt_cache_key` and session-affinity headers, preserve the unchanged request prefix, and report provider-returned cached-input tokens and their reduced cost separately.
- Persist and replay Pi's complete Responses items, not approximations: full reasoning-item JSON in `thinkingSignature`, assistant message ID/phase in `textSignature`, `call_id|item_id` tool IDs, and rich text/image `function_call_output` blocks. These fields are required both for valid follow-up requests and for stable cache prefixes.
...
Settings provides native browser login and device-code login, cancellation, status, and logout. Remove all credential-import commands and endpoints; do not read another application's credentials. Preserve existing BashKitten credentials until a replacement login succeeds. Use Pi's client ID, scopes, PKCE, state, localhost:1455 callback, manual authorization-code/redirect-URL fallback, device polling and token exchange. PKCE secrets and provider credentials remain exclusively in Rust. The temporary loopback OAuth callback is the only unauthenticated state-changing GET exception: it accepts only the pending random OAuth state, never exposes tokens, and closes after completion/cancellation. Browser attempts expire after 15 minutes as a local security/lifecycle bound. Login control endpoints require local Web authentication and CSRF. Redact token-bearing provider error bodies. Logout cancels pending login and removes only the OpenAI subscription credential under the same cross-process lock as refresh; an in-flight refresh must never restore logged-out credentials.
