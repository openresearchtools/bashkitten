...
<section id="app" class="hidden"><aside><header><strong>🐈 BashKitten</strong><button id="newBtn">＋</button><button id="settingsBtn">⚙</button></header><div id="sessions" class="sessions"></div></aside><main><header><strong id="title">Select a session</strong><span id="usage" class="usage"></span><button id="stopBtn" class="hidden">Stop</button></header><div id="chat"></div><form id="composer"><textarea id="prompt" placeholder="Message the agent…"></textarea><button class="primary">Send</button><div class="toolbar"><select id="delivery"><option value="queue">Queue</option><option value="steer">Steer current turn</option></select><input id="files" type="file" accept="image/*" multiple><select id="model"></select><select id="thinking"></select></div></form></main></section>
...
async function loadSessions(){let x=await api('/api/sessions?limit=100');$('#sessions').innerHTML=x.sessions.map(s=>`<div class="session" data-id="${s.id}"><b>${escapeHtml(s.title)}</b><small>${s.running?'<span class="dot">● running</span> · ':''}${escapeHtml(s.model||'')} · ${escapeHtml(s.cwd||'')}</small></div>`).join('');document.querySelectorAll('.session').forEach(e=>e.onclick=()=>openSession(x.sessions.find(s=>s.id===e.dataset.id)))}async function openSession(s){current=s;if(source)source.close();document.querySelectorAll('.session').forEach(e=>e.classList.toggle('active',e.dataset.id===s.id));$('#title').textContent=s.title;$('#stopBtn').classList.toggle('hidden',!s.running);$('#chat').innerHTML='';let h=await api(`/api/sessions/${s.id}/segments/${s.current_segment}`);h.entries.forEach(render);$('#chat').scrollTop=$('#chat').scrollHeight;if(s.running){source=new EventSource(`/api/sessions/${s.id}/events`);source.onmessage=e=>{try{render(JSON.parse(e.data))}catch{}}}}function render(e){if(e.type==='session')return;let role=e.role||e.message?.role||e.type||'event',content=e.content??e.message?.content??e.message??e;if(Array.isArray(content))content=content.map(x=>x.text||x.content||`[${x.type}]`).join('\n');if(typeof content!=='string')content=JSON.stringify(content,null,2);let d=document.createElement('div');d.className='msg '+(role.includes('tool')?'tool':role);d.textContent=content;$('#chat').append(d);$('#chat').scrollTop=$('#chat').scrollHeight;if(e.usage)$('#usage').textContent=`${e.usage.totalTokens||0} tokens`}function escapeHtml(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
$('#composer').onsubmit=async e=>{e.preventDefault();if(!current)return;let f=new FormData();f.append('content',$('#prompt').value);f.append('delivery',$('#delivery').value);for(let x of $('#files').files)f.append('file',x);$('#prompt').value='';try{await api(`/api/sessions/${current.id}/messages`,{method:'POST',body:f})}catch(x){alert(x.message)}};$('#newBtn').onclick=()=>$('#newDialog').showModal();document.querySelectorAll('[data-close]').forEach(x=>x.onclick=()=>x.closest('dialog').close());$('#newForm').onsubmit=async e=>{e.preventDefault();let f=new FormData();f.append('prompt',$('#newPrompt').value);f.append('cwd',$('#newCwd').value);f.append('model',$('#newModel').value);f.append('thinking',$('#newThinking').value);for(let x of $('#newFiles').files)f.append('file',x);try{let r=await api('/api/sessions',{method:'POST',body:f});$('#newDialog').close();await loadSessions();document.querySelector(`.session[data-id="${r.id}"]`)?.click()}catch(x){alert(x.message)}};$('#stopBtn').onclick=async()=>{if(current){await api(`/api/sessions/${current.id}/stop`,{method:'POST'});await loadSessions()}};
